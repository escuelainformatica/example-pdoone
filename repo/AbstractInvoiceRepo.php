 ...
 * Class AbstractInvoiceRepo
 * <pre>
 * $code=$pdoOne->generateCodeClass('invoices','example\repo',array(),array('customers'=>'CustomerRepo','invoice_details'=>'InvoiceDetailRepo','invoices'=>'InvoiceRepo',),array(),'','','BaseRepo','example\model\Invoice',array(),array());
 * </pre>
 */
abstract class AbstractInvoiceRepo extends BaseRepo
{
    ...
    public static function getDef($column=null,$filter=null) {
       $r = [
		    ...
		    'Date' => [
		        ...
		        'null' => TRUE,
		        'identity' => FALSE,
		        'sql' => 'datetime default CURRENT_TIMESTAMP DEFAULT_GENERATED'
		    ],
		    'IdCustomer' => [
		        'phptype' => 'int',
		        'conversion' => NULL,
    ...
    public static function convertOutputVal(&$row) {
		...
		!isset($row['Date']) and $row['Date']=null; // datetime
		!isset($row['IdCustomer']) and $row['IdCustomer']=null; // int
		isset($row['_IdCustomer'])
            and $row['_IdCustomer']['IdCustomer']=&$row['IdCustomer']; // linked MANYTOONE
    ...
    public static function getDefFK($structure=false) {
        if ($structure) {
            return [
			    'IdCustomer' => 'FOREIGN KEY REFERENCES`customers`(`IdCustomer`)'
			];
        }
    ...
    public static function toList($filter=null,$filterValue=null) {
       if(self::$useModel) {
            return Invoice::fromArrayMultiple( self::_toList($filter, $filterValue));
        }
    ...
    public static function first($pk = null) {
        if(self::$useModel) {
            return Invoice::fromArray(self::_first($pk));
        } 
    ...
    public static function factory($values = null, $recursivePrefix = '') {
        ...
        $row= [
		'IdInvoice'=>0,
		'_invoice_details'=>(in_array($recursivePrefix.'_invoice_details',$recursive,true))
		                            ? [] 
		                            : null, /* ONETOMANY! */
		'Date'=>'',
		'IdCustomer'=>0,
		'_IdCustomer'=>(in_array($recursivePrefix.'_IdCustomer',$recursive,true)) 
		                            ? CustomerRepo::factory(null,$recursivePrefix.'_IdCustomer') 
		                            : null, /* MANYTOONE!! */
		];
		isset($row['_IdCustomer'])
            and $row['_IdCustomer']['IdCustomer']=&$row['IdCustomer']; // linked MANYTOONE
    ...
    public static function factoryNull($values=null) {
        $row= [
		'IdInvoice'=>null,
		'_invoice_details'=>null, /* ONETOMANY! */
		'Date'=>null,
		'IdCustomer'=>null,
		'_IdCustomer'=>null, /* MANYTOONE!! */
		];
        if ($values !== null) {
            $row = array_merge($row, $values);
        }    
