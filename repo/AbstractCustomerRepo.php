 ...
 * <pre>
 * $code=$pdoOne->generateCodeClass('customers','example\repo',array(),array('customers'=>'CustomerRepo','invoice_details'=>'InvoiceDetailRepo','invoices'=>'InvoiceRepo',),array(),'','','BaseRepo','example\model\Customer',array(),array());
 * </pre>
 */
abstract class AbstractCustomerRepo extends BaseRepo
{
    const TABLE = 'customers';
    const IDENTITY = 'IdCustomer';
    const PK = [
	    'IdCustomer'
	];
    const ME=__CLASS__;
    ...
    public static function toList($filter=null,$filterValue=null) {
       if(self::$useModel) {
            return Customer::fromArrayMultiple( self::_toList($filter, $filterValue));
        }
    ...
    public static function first($pk = null) {
        if(self::$useModel) {
            return Customer::fromArray(self::_first($pk));
        } 
    ...
    public static function factory($values = null, $recursivePrefix = '') {
        ...
        $row= [
		'IdCustomer'=>0,
		'_invoices'=>(in_array($recursivePrefix.'_invoices',$recursive,true))
		                            ? [] 
		                            : null, /* ONETOMANY! */
		'Name'=>''
		];
