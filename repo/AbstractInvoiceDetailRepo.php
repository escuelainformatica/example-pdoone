 ...
 * Class AbstractInvoiceDetailRepo
 * <pre>
 * $code=$pdoOne->generateCodeClass('invoice_details','example\repo',array(),array('customers'=>'CustomerRepo','invoice_details'=>'InvoiceDetailRepo','invoices'=>'InvoiceRepo',),array(),'','','BaseRepo','example\model\InvoiceDetail',array(),array());
 * </pre>
 */
abstract class AbstractInvoiceDetailRepo extends BaseRepo
{
    const TABLE = 'invoice_details';
    const IDENTITY = 'IdInvoiceDetail';
    const PK = [
	    'IdInvoiceDetail'
	];
    const ME=__CLASS__;
    ...
    public static function convertOutputVal(&$row) {
		!isset($row['IdInvoiceDetail']) and $row['IdInvoiceDetail']=null; // int
		!isset($row['IdInvoice']) and $row['IdInvoice']=null; // int
		!isset($row['Product']) and $row['Product']=null; // varchar
		!isset($row['UnitPrice']) and $row['UnitPrice']=null; // decimal
		!isset($row['Quantity']) and $row['Quantity']=null; // int
		isset($row['_IdInvoice'])
            and $row['_IdInvoice']['IdInvoice']=&$row['IdInvoice']; // linked MANYTOONE
    ...
    public static function where($sql, $param = PdoOne::NULL)
    {
        ...
        return InvoiceDetailRepo::class;
    }

    public static function getDefFK($structure=false) {
        if ($structure) {
            return [
			    'IdInvoice' => 'FOREIGN KEY REFERENCES`invoices`(`IdInvoice`)'
			];
        }
    ...
    public static function toList($filter=null,$filterValue=null) {
       if(self::$useModel) {
            return InvoiceDetail::fromArrayMultiple( self::_toList($filter, $filterValue));
        }
    ...
    public static function setRecursive($recursive=[])
    {
        if(is_string($recursive)) {
            $recursive=InvoiceDetailRepo::getRelations($recursive);
        }
    ...
    public static function first($pk = null) {
        if(self::$useModel) {
            return InvoiceDetail::fromArray(self::_first($pk));
        } 
    ...
    public static function factory($values = null, $recursivePrefix = '') {
		...
		'IdInvoiceDetail'=>0,
		'IdInvoice'=>0,
		'_IdInvoice'=>(in_array($recursivePrefix.'_IdInvoice',$recursive,true)) 
		                            ? InvoiceRepo::factory(null,$recursivePrefix.'_IdInvoice') 
		                            : null, /* MANYTOONE!! */
		'Product'=>'',
		'UnitPrice'=>0.0,
		'Quantity'=>0
